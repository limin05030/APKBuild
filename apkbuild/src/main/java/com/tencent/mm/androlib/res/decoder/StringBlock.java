...
import static org.gradle.internal.impldep.org.bouncycastle.crypto.tls.CipherType.block;
...
public class StringBlock {
    ...
    public static int writeTableNameStringBlock(ExtDataInput reader, ExtDataOutput out, Map<Integer, String> tableProguardMap, boolean logEnable) throws IOException, AndrolibException {
        int type = reader.readInt();
        int chunkSize = reader.readInt();
        int stringCount = reader.readInt();
        int styleOffsetCount = reader.readInt();
        ...
        if (logEnable) {
            if (block.m_isUTF8) {
                System.out.println("resources.arsc Character Encoding: utf-8");
            } else {
                System.out.println("resources.arsc Character Encoding: utf-16");
            }
        }
