...
public class RawARSCDecoder {
    ...
    private RawARSCDecoder(InputStream arscStream, boolean logEnable) throws AndrolibException, IOException {
        mIn = new ExtDataInput(new LEDataInputStream(arscStream));
        mExistTypeNames = new HashMap<>();
        this.logEnable = logEnable;
    }

    public static ResPackage[] decode(InputStream arscStream, boolean logEnable)
            throws AndrolibException {
        try {
            RawARSCDecoder decoder = new RawARSCDecoder(arscStream, logEnable);
            if (logEnable) {
                System.out.println("parse to get the exist names in the resouces.arsc first\n");
            }
    ...
    private void readLibraryType() throws AndrolibException, IOException {
        ...
        for (int i = 0; i < libraryCount; i++) {
            ...
            if (logEnable) {
                System.out.println(String.format(Locale.getDefault(), "Decoding Shared Library (%s), pkgId: %d\n", packageName, packageId));
            }
        }
